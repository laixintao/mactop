...
def refresh_callback(*_):
    gpu_freq = metrics.get_hardware().m1_gpu.freq_hz
    return gpu_freq
...
class GPUFreqText(BaseStatic):
    ...
    def compose(self) -> ComposeResult:
        yield DynamicText(
            ...
            value_render_fn=lambda x: f"{x / 1_000_000:.0f} MHz",
...
def get_gpu_usage():
    idle = metrics.get_hardware().m1_gpu.idle_ratio
    if idle is None:
        return None
    busy = 1 - idle
    return [busy, idle]
...
def get_gpu_energy():
    return metrics.get_hardware().power_history.get("gpu")


class M1GPUEnergyPanel(PowerPanel):
    BORDER_TITLE = "GPU Energy"

    def __init__(self, label="GPU Power", show_value=True, *args, **kwargs):
        super().__init__(
            component="gpu", label=label, show_value=show_value, *args, **kwargs
        )
